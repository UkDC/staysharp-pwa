...
- Любые `add/update/delete` пишутся в outbox-очередь (`staysharp_cloud_outbox`).
...
- Кнопка `clearCache` в боковой панели очищает только локальный кеш справочника и пересобирает его из встроенной базы.
- Это не полный сброс браузерного кеша и не удаление журнала.

## Обновление PWA
- Кнопка `update PWA` в боковой панели очищает web cache / Service Worker cache и перезапускает приложение.
- Это основной способ подтянуть свежую версию на iPhone и iPad, где обычный reload часто недостаточен.
