...
- Кнопка `dbSync` в разделе Журнал запускает ручную синхронизацию журнала с облаком.
...
- Кнопка `dbSync` в разделе Справочник подтягивает актуальный лист `Database` из Google Sheets.
...
- Для автоматического заполнения `ID / Date / UpdatedAt` при ручном вводе строк рекомендуется включить `onEdit`.
...
- `dbSync` в Журнале обнуляет локальный журнал:
