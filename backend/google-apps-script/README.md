...
- При чтении `History` backend умеет автоматически добивать отсутствующие `ID`, `Date`, `UpdatedAt` в legacy-строках.
...
Примечание:
- `onEdit` остаётся рекомендуемым способом для ручного ввода строк;
- но даже если старые строки уже были добавлены без технических полей, чтение `History` попробует восстановить их автоматически.

Лист `Database` также должен существовать: он используется для загрузки справочника через pull-down обновление в разделе Справочник.
...
3. Выполните pull-down обновление в разделе Журнал или дождитесь фоновой отправки.
